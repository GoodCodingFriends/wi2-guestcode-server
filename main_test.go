...
To: Agent <agent@wi2-guestcode-server.appspotmail.com>
...
func TestReceivingMail(t *testing.T) {
	...
	req, err := inst.NewRequest("POST", "/_ah/mail/agent@wi2-guestcode-server.appspotmail.com", strings.NewReader(mailMsgStub))
	if err != nil {
		t.Fatal(err)
	}
